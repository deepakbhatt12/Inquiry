...
", Group = "sku", Rating = 1)]
        ...
        {
            var sku = _repos.Value.GetSku(id);
        ...
        public virtual ActionResult GetRecentOrders(int? skuId)
        {
            if (skuId == null)
            {
                throw new ArgumentNullException("Both customer and skuId cannot be null");
            }
            var orders = new PoListViewModel
            {
            ...
            return PartialView(MVC_Inquiry.Inquiry.SharedViews.Views._poListPartial, orders);
        }
