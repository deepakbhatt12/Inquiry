...
namespace DcmsMobile.Inquiry.Areas.Inquiry.ReturnEntity
{
    internal class ReturnEntityRepository:IDisposable
    {
        ...
        /// <returns></returns>
        public IList<ReturnReceipt> GetReturnInfo(string returnNumber, int maxRows)
        {
            const string QUERY = @"
            ...
            var binder = SqlBinder.Create(row => new ReturnReceipt
            {
        ...
        public IEnumerable<ReturnReceipt>  GetReturnReceiptInfo(string receiptNumber)
        {
            const string QUERY = @"
