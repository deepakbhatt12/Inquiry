...
SELECT Q1.CARTON_ID,
  ...
  LEFT OUTER JOIN Q2
   ...
   and q2.sku_vwh_seq = 1
        ...
        public IList<ActiveCarton> GetCartonList()
        {
            Contract.Assert(_db != null);
            const string QUERY = @"
                                SELECT S.CARTON_ID    AS CARTON_ID,
                                       ...
                                       S.VWH_ID       AS VWH_ID,
                                       S.QUALITY_CODE AS QUALITY_CODE, 
                                       S.PRICE_SEASON_CODE AS PRICE_SEASON_CODE,                                     
                                       SD.QUANTITY AS QUANTITY,
                                       SD.SKU_ID AS SKU_ID,
                                       SD.STYLE AS STYLE,
                                       SD.COLOR AS COLOR,
                                       SD.DIMENSION AS DIMENSION,
                                       SD.SKU_SIZE AS SKU_SIZE,
                                       S.DAMAGE_CODE AS DAMAGE_CODE,
                                       TSP.SEWING_PLANT_CODE AS SEWING_PLANT_CODE,
                                       TSP.SEWING_PLANT_NAME AS SEWING_PLANT_NAME  
                                  FROM SRC_CARTON S
                                  INNER JOIN SRC_CARTON_DETAIL SD
                                  ON S.CARTON_ID=SD.CARTON_ID
                                  LEFT OUTER JOIN TAB_SEWINGPLANT TSP
                                   ON S.SEWING_PLANT_CODE = TSP.SEWING_PLANT_CODE
                                    ";
