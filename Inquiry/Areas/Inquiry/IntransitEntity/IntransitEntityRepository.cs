...
namespace DcmsMobile.Inquiry.Areas.Inquiry.IntransitEntity
{
    ...
    internal class IntransityEntityRepository : IDisposable
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IList<IntransitShipmentSkuSummary> GetInboundShipmentSkuDetail(DateTime? minCloseDate, DateTime? maxCloseDate, ShipmentFilters statusFilter,string sewingPlant, int nRowsToShow)
        {
            Contract.Assert(_db != null);
            const string QUERY = @"
 ...
 order by ASH.max_upload_date desc nulls last";
        ...
        internal IList<SewingPlant> GetSewingPlantList()
        {
            ...
            const string QUERY = @"
                                    ...
                                    INNER JOIN <proxy />SRC_CARTON_INTRANSIT SCT
                                        ON SCT.SEWING_PLANT_CODE = SP.SEWING_PLANT_CODE
                                    WHERE SCT.ORIGINAL_SHIPMENT_ID IS NOT NULL
                                     GROUP BY SP.SEWING_PLANT_CODE
                                     ORDER BY SP.SEWING_PLANT_CODE
                                    ";
